...
func main() {
	...
	pack := &Package{
		...
		TagLength:      4,
		Tag:            []byte("demo"),
		Msg:            []byte(("现在林时间是:" + time.Now().Format("2006-01-02 15:04:05"))),
	}
	pack.Length = 8 + 2 + pack.HostnameLength + 2 + pack.TagLength + int16(len(pack.Msg))

	// 打印对象各字段的长度
	fmt.Printf("Version: %d, Length: %v, Timestamp: %d, HostnameLength: %d, Hostname: %d, TagLength: %d, Tag: %d, Msg: %d\n",
		len(pack.Version),
		pack.Length,
		pack.Timestamp,
		pack.HostnameLength,
